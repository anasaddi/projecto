 ...
 * ----------------------------------------------------------------------
 */
const STORAGE_KEY = 'km-dashboard-v3';
...
function buildDefaultState() {
  ...
  const mk = (title, done = false) => ({ id: uid('task'), title, done, children: [] });
...
function loadState() {
  ...
  try {
    const raw = localStorage.getItem(STORAGE_KEY) || localStorage.getItem('km-dashboard-v2') || localStorage.getItem('km-dashboard-v1');
...
function createTaskNode(title) {
  return { id: uid('task'), title: title.trim(), done: false, children: [] };
}
...
function FocusHeatmap({ dailyTaskLogs, prayerLogs, dailyCompletionLog, activeHabits, now }) {
  ...
  const heatmapDays = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      days.push({ key, score, isToday: key === toDateKey(now), habitsDone, prayersDone, tasksDone });
    }
    return days;
  ...
  return (
    <div className="bg-white dark:bg-[#1a1d24] border border-gray-200 dark:border-gray-800 rounded-xl p-3 flex flex-col gap-2 select-none">
      ...
      <div className="grid grid-cols-10 gap-0.5">
        {heatmapDays.map(({ key, score, isToday, habitsDone, prayersDone, tasksDone }) => (
          <div
            key={key}
            title={`${key}: ${Math.round(score * 100)}% · Habits ${habitsDone}/${activeHabits.length} · Prayers ${prayersDone}/5 · Tasks ${tasksDone}/3`}
...
function DenseTaskNode({ node, depth, projectId, projectAccent, onToggle, onDelete, onRename, onAddChild, onAddToTop3 }) {
        ...
        <div className={`ml-5 pl-2 border-l-2 ${accentBorder(projectAccent)} flex flex-col`}>
          {node.children.map((child) => (
            <DenseTaskNode
              ...
              onToggle={onToggle} onDelete={onDelete} onRename={onRename} onAddChild={onAddChild} onAddToTop3={onAddToTop3}
            />
          ))}
        </div>
      )}
...
export default function DashboardV3() {
  ...
  const xpData = useMemo(() => {
    let total = 0;
    const keys = Object.keys(dailyTaskLogs);
    keys.forEach(k => {
      const tl = dailyTaskLogs[k] || {};
      const pl = prayerLogs[k] || {};
      const cl = dailyCompletionLog[k] || { quick: [], project: [] };
      const h = activeHabits.reduce((a, t) => a + (tl[t.id] ? 1 : 0), 0);
      const p = PRAYERS.reduce((a, x) => a + (pl[x] ? 1 : 0), 0);
      const t = Math.min(3, (cl.quick?.length || 0) + (cl.project?.length || 0));
      total += h * 10 + p * 10 + t * 15;
    });
    const level = Math.floor(total / 100) + 1;
    const xpInLevel = total % 100;
    const titles = ['Rookie', 'Builder', 'Achiever', 'Master', 'Legend', 'Architect'];
    const title = titles[Math.min(level - 1, titles.length - 1)];
    return { total, level, xpInLevel, title };
  ...
  useEffect(() => {
    if (top3DoneCount === 3) confetti({ particleCount: 120, spread: 70, origin: { y: 0.6 } });
  }, [top3DoneCount]);

  const [isDeepWork, setIsDeepWork] = useState(false);

  // Actions
  const toggleDailyTask = (id, val) => { setDailyTaskLogs(p => ({ ...p, [todayKey]: { ...p[todayKey], [id]: val } })); if (val) playPopSound(); };
  const togglePrayer = (name, val) => { setPrayerLogs(p => ({ ...p, [todayKey]: { ...p[todayKey], [name]: val } })); if (val) playPopSound(); };
  ...
  return (
    <div className="h-full w-full bg-gray-50 dark:bg-[#0B0F19] text-gray-900 dark:text-gray-100 flex flex-col overflow-hidden font-sans select-none selection:bg-indigo-500/30">
      ...
      <header className="shrink-0 px-4 py-2.5 flex items-center justify-between bg-white dark:bg-[#1a1d24] border-b border-gray-200 dark:border-gray-800 select-none">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-extrabold tracking-tight flex items-center gap-2">
            Dashboard
            <span className="text-[10px] font-bold px-1.5 py-0.5 bg-gradient-to-r from-emerald-500/20 to-indigo-500/20 text-emerald-600 dark:text-indigo-400 rounded uppercase tracking-widest">GOD-TIER</span>
          </h1>
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400">Lv.{xpData.level} {xpData.title}</span>
            <div className="w-20 h-1 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
              <div className="h-full bg-amber-500 rounded-full" style={{ width: `${xpData.xpInLevel}%` }} />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
           ...
           <button
             ...
             className={`text-[10px] font-bold px-2 py-1 rounded transition-colors ${isDeepWork ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/20'}`}
      ...
      <AnimatePresence>
        {isDeepWork && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-black/80 flex flex-col items-center justify-center gap-8"
            onClick={() => setIsDeepWork(false)}
          >
            <div onClick={(e) => e.stopPropagation()} className="flex flex-col items-center gap-8">
              <PomodoroCompact />
              {top3Resolved.find(s => s && !s.missing && !s.done) && (
                <p className="text-white/90 text-sm font-medium">Focus on: {top3Resolved.find(s => s && !s.missing && !s.done).title}</p>
              )}
            </div>
            <button onClick={() => setIsDeepWork(false)} className="text-white/60 hover:text-white text-xs">Click outside to exit Zen</button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex-1 min-h-0 p-4 grid grid-cols-1 md:grid-cols-12 gap-4 overflow-hidden">
        {/* COL 1 */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="md:col-span-3 flex flex-col gap-4 min-h-0">
          <PomodoroCompact />
        ...
        {/* COL 2 */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.05 }} className="md:col-span-4 flex flex-col gap-4 min-h-0">
              ...
              {dailyTaskTemplates.map(task => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 px-2.5 py-2 rounded-lg border transition-all select-text ${isLocked ? 'opacity-50 bg-gray-50 dark:bg-gray-800/50 border-transparent cursor-default' : isDone ? 'bg-gray-50 dark:bg-white/5 border-gray-200 dark:border-gray-700/50 cursor-pointer' : 'bg-transparent border-transparent hover:bg-gray-50 dark:hover:bg-white/5 cursor-pointer'}`}
                  >
                    {isLocked ? (
        ...
        {/* COL 3 */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }} className="md:col-span-5 bg-white dark:bg-[#1a1d24] border border-gray-200 dark:border-gray-800 rounded-xl p-3.5 flex flex-col min-h-0">
